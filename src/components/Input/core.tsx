...
import type { FieldError, RegisterOptions, UseControllerProps } from "react-hook-form";

export type BaseInputProps = Omit<HTMLAttributes<HTMLInputElement>, 'name' | 'class' | 'pattern' | 'required' | 'minLength' | 'maxLength' | 'disabled'> & {
...
const InputCore: FunctionComponent<InputCoreProps> = (
  { label, rules, leftAdornment, rightAdornment, className, disabled, placeholder, error, ...props }, ref) => {
  const id = useId();
  return (
    <div className="w-full">
      {label ? (
        <label
          ...
          {label}
          {rules?.required ? (
            // <Typography className="text-danger-600" variant="medium/medium">
            //   *
            // </Typography>
            "*"
          ) : (
      ...
      {/* <Typography variant="small/normal" className="text-danger-600 mt-0.5"> */}
      {error ? error.message : ''}
      {/* </Typography> */}
    </div>
  )
