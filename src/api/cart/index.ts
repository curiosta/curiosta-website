...
type TLoadableOptionsList = {
  cart: 'get' | 'create' | 'update' | 'reset' | { line_items: 'get' | 'add' | 'remove' | 'update' | { quantity: '' }, shipping: 'all' | 'set' }
};
...
class CartStore {
  ...
  async updateCart(payload: TCartUpdatePayload) {
    ...
    const updateResult = await medusa.carts.update(this.store.value.id, payload)
  ...
  async listShippingMethods() {
    ...
    const response = await medusa.shippingOptions.listCartOptions(this.store.value.id);
  ...
  async updateShippingMethod(id: string) {
    ...
    const result = await medusa.carts.addShippingMethod(this.store.value.id, { option_id: id, })
